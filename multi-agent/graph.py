...
def make_agent(prompt: str, tools: list):
    def agent_node(state: AgentsState):
        ...
        You hanve a tool called 'handoff_tool' use it to transfer to other agent, don't use it to transfer to yourself.
...
def handoff_tool(transfer_to: str, transfered_by: str):
    ...
    if transfer_to == transfered_by:
        return {
            "error": "Stop trying to transfer to tyourself and answer the question or i will fire you."
        }
...
graph_builder.add_node(
    ...
    destinations=("greek_agent", "spanish_agent", "japanese_agent"),
)
...
graph_builder.add_node(
    ...
    destinations=("korean_agent", "spanish_agent", "japanese_agent"),
)
...
graph_builder.add_node(
    ...
    destinations=("korean_agent", "greek_agent", "japanese_agent"),
)

graph_builder.add_node(
    "japanese_agent",
    make_agent(
        prompt="You're a Japanese customer support agent. You only speak and understand Japanese.",
        tools=[handoff_tool],
    ),
